...
vim.api.nvim_create_autocmd("User", {
	...
	callback = function()
		...
		map("n", "<leader>ot", "<cmd>ObsessTimer<CR>", { desc = "设置定时器(m)" })
		map("n", "<leader>os", "<cmd>ObsessTimerSec<CR>", { desc = "设置定时器(s)" })
		map("n", "<leader>ow", "<cmd>ObsessToggle<CR>", { desc = "显现隐藏窗口" })
		map("n", "<leader>oc", "<cmd>ObsessClose<CR>", { desc = "关闭窗口并停止计时" })
		map("n", "<leader>oa", "<cmd>ObsessTaskAdd<CR>", { desc = "添加任务" })
		map("n", "<leader>ox", "<cmd>ObsessTaskDone<CR>", { desc = "切换任务状态" })
		map("n", "<leader>od", "<cmd>ObsessTaskDel<CR>", { desc = "删除任务" })
		map("n", "<leader>oe", "<cmd>ObsessTaskClear<CR>", { desc = "清空任务列表" })
		map("n", "<leader>ol", "<cmd>ObsessTaskLoad<CR>", { desc = "加载任务面板" })
	end,
})
